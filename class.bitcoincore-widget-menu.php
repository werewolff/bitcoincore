...
class Bitcoincore_Menu_Widget extends WP_Widget
{
            ...
            foreach ($versions as $version) {
                ...
                foreach ($categories as $category) {
                    ...
                    if (!empty($methods_list))
                        $categories_list[] = '<ul><li>' . $category->name . '</li>' . $methods_list . '</ul>';
                }
                $categories_list = implode('', $categories_list);
                if (!empty($categories_list)) {
                    $version_page_link = get_page_link($version->page_id);
                    $versions_list[] = '<ul><li><a href="' . $version_page_link . '">' . $version->name . '</a></li>' . $categories_list . '</ul>';
                }
            }
            $versions_list = implode('', $versions_list);
            if (!empty($versions_list)) {
                $blockchain_page_link = get_page_link($blockchain->page_id);
                $blockchains_list[] = '<ul><li><a href="' . $blockchain_page_link . '">' . $blockchain->name . '</a></li>' . $versions_list . '</ul>';
            }
        }

        $content = implode('', $blockchains_list);
        $content_block = '<div class="menu-left">' . $content . '</div>';
    ...
    {
        wp_enqueue_style('bitcoincore-widget-menu-style', plugins_url('/assets/css/bitcoincore-plg-widget-menu.css', __FILE__));
        wp_enqueue_script('bitcoincore-widget-menu-script', plugins_url('/assets/js/bitcoincore-plg-widget-menu.js', __FILE__), array('jquery'));
    }
